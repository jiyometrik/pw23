...
# read the data
data = pd.read_csv('./data/Datafiniti_Hotel_Reviews.csv',
                   header=0, sep=',', on_bad_lines='skip')

# extract the title and body text of each review into a large list
bodies, titles = data['reviews.text'].astype(
    str), data['reviews.title'].astype(str)
...
bars_container = axes.bar(['Positive', 'Negative'], [
                          len(sentiments_pos), len(sentiments_neg)])
